"""
Together AI Provider Implementation for TangkuAgentOS AI Foundation Framework.
"""
...
class TogetherAIModel(AIModel):
    """Together AI implementation of the AIModel interface."""
    ...
    def capabilities(self) -> ModelCapabilities:
        if "llama-3" in self._model_name:
    ...
    async def stream_chat(
        ...
        **kwargs: Any,
        ...
        """Stream a chat completion using Together AI."""
    ...
    async def embed(self, text: str, **kwargs: Any) -> List[float]:
        """Generate embeddings (not supported for Together AI)."""
        raise NotImplementedError("Together AI does not support embeddings directly.")

    async def embed_batch(self, texts: List[str], **kwargs: Any) -> List[List[float]]:
        """Generate embeddings for a batch of texts (not supported for Together AI)."""
        raise NotImplementedError("Together AI does not support batch embeddings directly.")


class TogetherAIProvider(BaseProvider):
    """Together AI implementation of the BaseProvider interface."""
    ...
    async def get_model_capabilities(self, model_name: str) -> ModelCapabilities:
        """Get the capabilities of a specific Together AI model."""
        model = TogetherAIModel(model_name, self._api_key, self._base_url)
        return model.capabilities

    async def create_model(self, model_name: str, **kwargs: Any) -> AIModel:
        """Create an instance of a Together AI model."""
        return TogetherAIModel(model_name, self._api_key, self._base_url, **kwargs)

    async def health_check(self) -> bool:
        """Perform a health check for the Together AI provider."""
        return True
